...
class Database:
    ...
    def select_data_id(self, id):
        return self.execute(
            sql="SELECT * FROM Data WHERE id = ?", parameters=(id,), fetchone=True
        )

    def select_data_nom(self, nom):
        return self.execute(
            sql="SELECT * FROM Data WHERE nomi  = ?", parameters=(nom,), fetchone=True
        )

    def select_data_nomlar(self, nom):
        return self.execute(
            sql=f"SELECT * FROM Data WHERE nomi LIKE '%{nom}%'", fetchall=True
        )

    def select_data_turi_ordered(self, turi):
        return self.execute(
            sql="SELECT * FROM Data WHERE turi = ? ORDER BY status DESC",
            parameters=(turi,),
            fetchall=True,
        )
    ...
    def select_top(self):
        return self.execute(
            sql="SELECT * FROM Data ORDER BY status DESC", fetchall=True
        )

    def add_data(self, nomi, file_id, turi, status):
        self.execute(
            sql="INSERT INTO Data(nomi, file_id, turi, status) VALUES(?, ?, ?, ?)",
            parameters=(nomi, file_id, turi, status),
            commit=True,
        )
