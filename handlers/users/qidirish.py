...
async def qidieish(msg : types.Message, state : FSMContext):
    await msg.answer("<b>Qidirish uchun biror so'z yuboring : </b>", reply_markup=ortga.ortga)
...
async def qidir(msg : types.Message, state : FSMContext):
    if msg.text.isalpha():
        ...
        if natija == []:
            await msg.answer("<b>Hech nima topilmadi </b>🫤\n\n<i>Qayta urinib ko'ring : </i>", reply_markup=ortga.ortga)
        else:
            await msg.answer("<b>Topilgan natija(lar) : </b>", reply_markup=show_data.show_data(natija))
