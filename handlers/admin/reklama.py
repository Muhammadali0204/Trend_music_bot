...
from keyboards.default.admin_menu import menu
from keyboards.default.ortga import ortga
import asyncio
...
async def func(call : types.CallbackQuery, state : FSMContext):
    await call.message.answer("<b>Xabarni yuboring : <i>(Forward)</i></b>", reply_markup=ortga)
    await state.set_state("forward")
    await call.message.delete()
    
@dp.callback_query_handler(text="copy", state="reklama_turi")
async def func(call : types.CallbackQuery, state : FSMContext):
    await call.message.answer("<b>Xabarni yuboring : <i>(Copy)</i></b>", reply_markup=ortga)
